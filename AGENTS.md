...
- **Daily briefing news quality**: after collection, run a structured model editorial pass across article content to keep one representative per underlying story and reject semantic topic mismatches caused by keyword collisions; fall back to deterministic selection if that pass fails. Then render per-article one-sentence summaries after bounded selection (LLM rewrite with deterministic fallback), enforce a per-topic selection hard cap (currently 10) to avoid single-topic dominance, guarantee a small floor of general headlines when available (currently 3), and use confidence-scored topic matching with accent-insensitive normalization as an initial retrieval signal.
