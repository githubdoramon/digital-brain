...
- **Event participant list extraction**: contact extraction is LLM-only; do not short-circuit `/event` participant resolution with deterministic fast-path parsing. Explicit participant lists may contain lowercase nicknames (for example `with Marcela, paty and bebel`); keep those list entries as people. Additive follow-ups like "you forgot X" must not drop previously listed participants unless the user explicitly asks to remove/exclude them.
...
- **Single-candidate contact disambiguation**: a single fuzzy candidate that is not safe enough for deterministic direct resolution may still be accepted when the LLM disambiguator returns high-confidence `resolved`; multi-candidate ambiguity remains governed by `CONTACT_DISAMBIGUATION_STRICTNESS`.
