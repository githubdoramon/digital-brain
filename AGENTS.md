...
- **Daily briefing event quality**: event prep summaries should prioritize non-obvious, context-grounded guidance, use `summarize_memories` as the primary per-meeting history synthesis path, filter low-value generic advice, clearly separate current upcoming-event context from historical similar-event references, and use attendee-overlap fallback (exact set first, then >=80%) when title/recurrence history is weak.
