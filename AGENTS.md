...
- **Proposed events flow**: the `proposed_events_daily` worker runs after 15:50 UTC, scans same-day `user_location_history` in the user's latest captured timezone for 15+ minute stays with no blocking timed-event overlap, treats all-day/full-day events as non-blocking calendar context, skips home-like/ignored places, stores pending suggestions in `proposed_events`, expires them after 7 days, and sends `proposed_events_ready` push notifications that deep-link mobile to `/settings/proposed-events`. Run cheap eligibility and blocking-overlap checks before LLM enrichment; ambiguous timed overlaps should be disambiguated against the stay's place/activity so unrelated broad events do not block proposals. Before storing proposals with same-place history, retrieve recent same-place events, linked place contacts, and recurrence stats, then use the smart chat model with high reasoning effort plus structured JSON schema to enrich title/summary/people/confidence; validate LLM-suggested contact IDs against retrieved contacts only. Accepting a proposal must create a normal event; dismiss/ignore actions should preserve proposal audit state.
