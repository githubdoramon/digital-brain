...
- **User facts injection policy**: any LLM call site that produces user-facing content or makes decisions influenced by user preferences MUST inject user facts via `get_user_facts_context(user_email, query)`. Currently injected in: agent message builders (`main`, `memory_expert`), `/event` command entity extraction, and daily briefing pipeline (per-event synthesis + markdown generation). Utility LLM calls (title generation, tag suggestion, translation, validation) do NOT need facts. When adding a new LLM call site, ask: "Would knowing user preferences/traits change this output?" — if yes, inject facts.
