...
- **Daily briefing event quality**: event prep summaries should prioritize non-obvious, context-grounded guidance, filter low-value generic advice, clearly separate current upcoming-event context from historical similar-event references, use attendee-overlap fallback (exact set first, then >=80%) when title/recurrence history is weak, and enforce evidence-anchored prep bullets (drop unsupported claims).
...
- **Current-status progress queries should prefer newest aligned event evidence**: for questions like pregnancy progress, due dates, or other evolving status checks, completion should prefer the newest relevant event candidate over a higher-scoring but unrelated document hit, and forced follow-up prompts should carry the exact candidate ID/tool args instead of asking the model to reconstruct them.
