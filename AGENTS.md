...
- **Event participant list extraction**: explicit participant lists may contain lowercase nicknames (for example `with Marcela, paty and bebel`); keep those list entries as people. Additive follow-ups like "you forgot X" must not drop previously listed participants unless the user explicitly asks to remove/exclude them.
