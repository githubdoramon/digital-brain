...
**Mobile background location convention**: Android background location uses continuous capture with best-effort scheduled draining. Keep the location and drain task definitions imported from `mobile/index.js`; capture should still use the configured distance/time thresholds, but Android location-task callbacks should enqueue rather than upload immediately so the scheduled drain worker handles backend delivery. Preserve queue/drain/debug logging so exported logs can distinguish capture, enqueue, drain, auth, and backend failures. iOS may still drain directly from the location task where appropriate.
