...
- **Provider message normalization**: `llm_helpers.build_chat_payload()` must merge all system messages into one system message at index zero while preserving their order and preserving the order of all non-system messages; model chat templates such as Qwen3.8 reject later system messages.
...
- **Reasoning-capable structured output may require continuation**: use `call_llm_json_agentic` for bounded flows where a provider can return reasoning without final content or malformed JSON; preserve the requested reasoning effort, carry the prior attempt into a continuation turn, validate the task-specific result, and fall back only after the bounded turn budget is exhausted.
...
- **Daily briefing news quality**: after collection, partition articles into one bounded structured editorial curation bucket per tracked topic plus a general-news bucket (maximum 28 candidates per request). Process buckets synchronously and sequentially with a 300-second per-request timeout to avoid overloading local reasoning models. Keep one representative per underlying story within each bucket and reject semantic topic mismatches caused by keyword collisions; merge multi-topic articles by stable identity and controller-validate labels, fail closed for failed tracked-topic buckets, and retain only bounded general candidates on isolated general-bucket failure. Preserve final URL/cluster dedupe, deterministic ranking, and fallback summary generation. Then render per-article one-sentence summaries after bounded selection (LLM rewrite with deterministic fallback), enforce a per-topic selection hard cap (currently 10) to avoid single-topic dominance, guarantee a small floor of general headlines when available (currently 3), and use confidence-scored topic matching with accent-insensitive normalization as an initial retrieval signal.
...
- **Slash-command preview flows**: `/contact` mirrors `/event` with controller-validated extraction, preview UI directives, clarification follow-ups, and explicit confirmation before mutating contacts, relationships, or contact-place links. `/event` structured detail extraction/update and `/contact` extraction/update use the smart chat model with `xhigh` reasoning effort; contact resolution remains the fast/no-reasoning path and must normalize non-strict local-model JSON shapes before downstream parsing. When a nested `/event` person reference has no direct candidate list because its anchor is ambiguous, accept only a user-typed exact existing contact name or alias as the clarification answer; bind it to the original reference and leave partial/non-unique replies unresolved. `/contact` extraction should model plural graph operations (`contacts`, `relationships`, `contact_place_links`) rather than assuming a single main contact, preserve clarification conversation history plus prior extraction state across follow-ups, and prefer specific Title Case relationship labels/reciprocals when context supports them. Contact previews should use a single summary card with the event-style edit icon and full-screen draft editor, not inline edit forms.
- **Entity tag generation is asynchronous**: `/event` and `/contact` extraction/update prompts must not generate tags inline. Event, document, and contact persistence store only existing/user-provided tags, then `event_tag_jobs.py`, `document_tag_jobs.py`, or `contact_tag_jobs.py` queue post-save enrichment through `async_jobs`, sanitize LLM tag results, merge them into the current entity row, and refresh affected embeddings.
- **Event command timezone semantics**: `/event` extraction and confirmation must treat user-stated clock times as local wall times in `client_context.timezone` when available. Naive ISO datetimes from LLM extraction or mobile draft edits inherit that timezone; do not default them to UTC or convert a stated `10am` into a different displayed local hour.
- **Event meal-time inference**: when `/event` text gives a date and a meal but no clock time, prefer conventional local meal times over midnight: breakfast 08:30, brunch 11:00, lunch 12:30, dinner 19:30.
- **Event participant list extraction**: contact extraction is LLM-only; do not short-circuit `/event` participant resolution with deterministic fast-path parsing, and do not run a separate participant-pruning review after extraction. Explicit participant lists may contain lowercase nicknames (for example `with Marcela, paty and bebel`); keep those list entries as people. Additive follow-ups like "you forgot X" must not drop previously listed participants unless the user explicitly asks to remove/exclude them.
...
- **Slash-command restore parity**: `/event`, `/contact`, and future preview/clarification command state must be recoverable from persisted assistant metadata, not only process-local command storage. Mobile main-session and thread-detail restore should return the active unresolved preview/clarification ID so closed/backgrounded apps can recreate the latest command card. New command flows should persist enough confirmation data or a serializable `command_state` payload in `command_result`.
- **Location-aware place inference**: ask flows may enrich `client_context.location` with `inferred_location` (known-place proximity first, Geoapify reverse geocode fallback). For `/event`, never expose runtime coordinates to extraction. Preserve `where` when it was extracted from the user's text; otherwise current-place inference and coordinate-based place/geocode proximity are allowed only when a location capture is at most 30 minutes old and the event starts from 2 hours in the past through 30 minutes in the future, or its explicit interval contains now. Date-only and unknown-time events fail closed and keep `where` blank.
...
- **Single-candidate contact resolution**: if contact lookup returns exactly one candidate, resolve it directly without LLM disambiguation. LLM disambiguation is only for multi-candidate ambiguity and remains governed by `CONTACT_DISAMBIGUATION_STRICTNESS`.
...
- **Document enhancement lifecycle**: persist an upload placeholder before enhancement, expose `enhancement_status` (`pending`, `processing`, `complete`, `failed`) and `enhancement_error`, and process the full pipeline through `document_enhancement_jobs.py` with retry semantics. Keep user tags separate from this lifecycle state. Document list defaults to `document_date` descending with nulls last and supports `created_at` sorting; use `scripts/find_document_orphans.py` for a read-only report of both unreferenced storage files and database rows whose files are missing.
...
- **High-confidence contact identity answers may use pre-resolution evidence**: for contact name/identity questions, the goal validator may accept a final answer that contains the display name of a high-confidence pre-resolved contact without requiring another retrieval call. Contact details such as phone, email, and address still require retrieval.
- **Fact extraction is router-gated**: `IntentClassification.should_generate_facts` is `false` for deterministic request/command routes; LLM routing sets it to `true` only for durable first-person facts. An explicit false skips the extraction task, true bypasses only the short-message heuristic, and an absent value preserves the legacy heuristic.
- **New-thread titles are generated asynchronously**: schedule title generation after exchange persistence and update the database only if the title remains a default, so title inference never delays the answer or overwrites a user rename.
- **Readable fetched pages should proceed directly to synthesis**: `fetch_web_page` coverage is deterministic and must not spend a separate LLM validation call. Once readable content is marked satisfied, the controller's next model turn has tools disabled and synthesizes from that page.
...
- **Contact extraction before lookup**: contact mention extraction is LLM-only and feeds deterministic DB/rule lookup. Single-candidate lookup results resolve directly; multi-candidate ambiguity may use LLM disambiguation according to policy.
...
- **User facts extraction runs as BackgroundTask**: triggered after conversation persistence in `llm.py` via callbacks from `routes/chat.py`. It must never block or crash user-facing responses. The router can skip extraction for questions/requests; short/trivial messages and slash commands still skip unless routing explicitly marked a durable fact.
...
Keep uploaded capture IDs and filenames as durable queue tombstones after local
media cleanup so a later gallery listing cannot redownload the same pictures.
When Automatic scene capture is off, startup may mirror its diagnostic log but
must not backfill retained private photos into `Image Pipeline Temp`.
The recordings screen reconciles its index from the shared `Recordings` folder
on screen focus and app resume, importing new M4A files and removing stale rows.
...
`mobile/index.js` before Expo Router; v8 continuously runs a native Sherpa
keyword spotter and invokes the packaged on-device ONNX embedding verifier only
for keyword candidates. Preserve the verifier's real preceding PCM context,
