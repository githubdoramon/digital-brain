...
- **Slash-command restore parity**: `/event` and `/contact` preview/clarification state must be recoverable from persisted assistant metadata, not only process-local command storage. Mobile main-session and thread-detail restore should return the active unresolved preview/clarification ID so closed/backgrounded apps can recreate the latest command card.
