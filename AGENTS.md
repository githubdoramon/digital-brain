...
**Mobile background location convention**: Keep Android foreground-service restarts conservative. Background location may use a foreground service for reliability, but do not restart an already-running Android location task unless task options actually changed; preserve queue/drain debug logging so exported logs can distinguish capture, enqueue, drain, auth, and backend failures.
