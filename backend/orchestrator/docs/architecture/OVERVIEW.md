...
- `/event` structured detail extraction/update and `/contact` extraction/update use the smart chat model with `x-high` reasoning effort. Participant/contact resolution remains a separate fast/no-reasoning path and normalizes non-strict local-model JSON shapes before parsing.
