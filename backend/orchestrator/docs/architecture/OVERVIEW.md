...
- `/event` datetime extraction and confirmation preserve user-stated local wall times. Naive ISO datetimes from the LLM or mobile draft editor inherit `client_context.timezone` when available rather than defaulting to UTC.
...
- Proposed events are generated by the `proposed_events_daily` async worker after 15:20 UTC. The worker scans that local day of `user_location_history` using the user's latest captured timezone, creates medium/high-confidence `proposed_events` for stays of at least 15 minutes without overlapping events, ignores home-like/ignored places, expires pending proposals after 7 days, and sends a push notification with `kind=proposed_events_ready` when new proposals are ready. Before storage, proposals with same-place history are enriched from recent events, linked place contacts, and recurrence stats through the smart chat model with high reasoning effort and a structured response schema; suggested contacts are validated against retrieved contact IDs. Mobile review APIs live under `/mobile/proposed-events`; accepting a proposal creates a normal event.
