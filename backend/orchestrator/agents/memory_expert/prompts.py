...
def get_memory_expert_system_prompt(search_limit: int = 30) -> str:
    ...
    return (
        ...
        "Use tags as precision hints when the user names a clear domain like work, health, finance, legal, family, or education, but do not require tags when semantic retrieval is already specific. "
...
def get_memory_expert_protocol_prompt() -> str:
    ...
    return (
        ...
        "- For recap/report questions with explicit time windows (e.g. 'summarize my work last week', 'important topics from meetings this month'):\n"
        "  * Prefer structured retrieval first: use summarize_memories when available, or get_events(action=by_time_span) with precise bounds and domain hints.\n"
        "  * If the user names a domain such as work/health/finance, use matching tags as a precision hint when likely available, but do not block on tags.\n"
        "  * After collecting the bounded event/document set, use semantic retrieval only to deepen that bounded scope, not as the first broad search.\n"
        "- For broad exploration or topic discovery without a strict recap window, semantic search remains first-class and can be the first step.\n"
        ...
        "- For work recap questions, documents can carry the important outcomes; inspect relevant documents alongside events before answering about decisions, outcomes, or follow-ups.\n"
        ...
        "  * summarize_memories: bounded recap tool for time-window summaries that combines events and documents into grounded topics, outcomes, and follow-ups. Prefer it for recap/report style questions.\n"
        ...
        "  * lookup_place_contacts: resolve who is linked to a place (e.g. 'who lives at X', 'who works at X').\n"
